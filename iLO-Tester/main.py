...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Lightweight HP iLO tester")
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in as_completed(futures):
            r = future.result()
            results.append(r)
            marker = "[green]HIT[/green]" if r.is_ilo else "[dim]--[/dim]"
            console.print(f"  [{len(results):>3}/{len(targets)}] {marker} [cyan]{r.ip}[/cyan]")

    results.sort(key=lambda r: ipaddress.IPv4Address(r.ip))
    visible = [r for r in results if r.is_ilo] if args.only_found else results

    table = Table(title="iLO Scan Results", box=box.ROUNDED, show_lines=True, highlight=True)
    table.add_column("IP",    style="cyan", no_wrap=True)
    table.add_column("URL",   style="blue", no_wrap=True)
    table.add_column("iLO?",  justify="center")
    table.add_column("Auth",  justify="center")

    for r in visible:
        ilo_col = "[green]YES[/green]" if r.is_ilo else "[dim]no[/dim]"
        ...
        if r.error:
            ...
            auth_col = f"[bold red]SUCCESS[/bold red] ({r.winner[0]}:{r.winner[1]})"
        else:
            auth_col = "[yellow]failed[/yellow]"

        table.add_row(r.ip, r.url or "—", ilo_col, auth_col)
