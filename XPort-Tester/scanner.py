...
_CONFIG_PATH = "/secure/ltx_conf.htm"
MAX_ATTEMPTS = 2

_XPORT_PATTERNS = re.compile(
...
def _looks_like_xport(response: requests.Response) -> bool:
    if _XPORT_PATTERNS.search(response.text):
        return True
    headers_str = " ".join(f"{k}: {v}" for k, v in response.headers.items())
    return bool(_XPORT_PATTERNS.search(headers_str))
...
def _try_login(base_url: str, session: requests.Session) -> tuple[bool, str | None, str | None]:
    ...
    for i, cred in enumerate(creds):
        ...
        if r.status_code == 200 and _looks_like_xport(r):
...
def scan(ip: str, check_auth: bool = False) -> ScanResult:
    ...
    is_target = _looks_like_xport(response)

    # Fallback: probe the XPort-specific protected path.
    # A 401 here is itself a strong signal — only XPort serves /secure/ltx_conf.htm.
    if not is_target:
        probe = _fetch(url.rstrip("/") + _CONFIG_PATH, session)
        if probe is not None:
            if _looks_like_xport(probe):
                is_target = True
            elif probe.status_code == 401:
                logging.debug("  [%s] %s returned 401 — XPort-specific path", ip, _CONFIG_PATH)
                is_target = True

    logging.info("  [%s] XPort detected: %s", ip, is_target)
